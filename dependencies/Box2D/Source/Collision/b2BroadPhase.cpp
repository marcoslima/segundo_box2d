...
uint16 b2BroadPhase::CreateProxy(const b2AABB& aabb, void* userData)
{
	...
	for (int32 axis = 0; axis < 2; ++axis)
	{
		...
		std::memmove(bounds + upperIndex + 2, bounds + upperIndex, (boundCount - upperIndex) * sizeof(b2Bound));
		std::memmove(bounds + lowerIndex + 1, bounds + lowerIndex, (upperIndex - lowerIndex) * sizeof(b2Bound));
